...
const bindEventListeners = () => {
    ...
    root.addEventListener('click', async (e) => {
        ...
        if (target.closest('.dropdown-link[data-action="logout"]')) await handlers.handleLogout();
...
const initializeApp = async () => {
    ...
    try {
        ...
        await auth.initializeMsal(); // Must be before loadState if loadState depends on auth
        Logger.log("MSAL initialized.");
        ...
        if (state.branding.enableMicrosoftLogin && !auth.isMsalConfigured) {
            const warningMsg = "Microsoft login is enabled but not configured. Please contact an administrator.";
        ...
        Logger.log("Handling auth redirect...");
        await auth.handleRedirect();
        const account = auth.getActiveAccount();
        Logger.log(`Auth redirect handled. Active account found: ${!!account}`);

        if (account) {
            Logger.log("Fetching user profile from Graph API.");
            const user = await auth.getGraphProfile();
            state.currentUser = user;
            Logger.log("User profile loaded.", { user: user.email });
