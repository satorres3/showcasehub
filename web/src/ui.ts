...
import { generateQuestionsFromKnowledge } from "./api";
    ...
    DOM.containers.modelSelect.innerHTML = `
        ...
        <div class="select-options hidden" role="listbox">
            ${container.availableModels.map(modelId => {
                ...
                return m ? `<div class="select-option ${modelId === container.selectedModel ? 'selected' : ''}" role="option" data-value="${m.id}">${m.icon} <span>${m.id}</span></div>` : '';
            }).join('')}
        </div>
    `;
    ...
    DOM.containers.personaSelect.innerHTML = `
        ...
        <div class="select-options hidden" role="listbox">
            ${container.availablePersonas.map(p => `
                <div class="select-option ${p === container.selectedPersona ? 'selected' : ''}" role="option" data-value="${p}">${personaIcon} <span>${p}</span></div>
            `).join('')}
        </div>
    `;
};
...
export const renderBranding = () => {
    ...
    if (DOM.buttons.microsoftLogin) {
        const showMicrosoftLogin = branding.enableMicrosoftLogin && isMsalConfigured;
...
export const renderKnowledgeFiles = () => {
    ...
    if (DOM.textElements.knowledgeSubtitle) {
        DOM.textElements.knowledgeSubtitle.textContent = container.isKnowledgeBasePublic ?
            ...
            'These files are used to ground the assistant and are private to this container.';
    }
    ...
    container.knowledgeBase.forEach(file => {
        ...
        item.querySelector('.delete-file-btn')?.addEventListener('click', (e) => {
            ...
            if (state.currentContainerId) {
                handleDeleteFileClick(state.currentContainerId, file.name);
            }
        });
...
export const renderContainerSettings = (containerId: string) => {
    ...
    const colorInputs = [
        { input: DOM.inputs.userBgColor, key: 'userBg' }, { input: DOM.inputs.userTextColor, key: 'userText' },
        { input: DOM.inputs.botBgColor, key: 'botBg' }, { input: DOM.inputs.botTextColor, key: 'botText' },
        { input: DOM.inputs.bgGradientStartColor, key: 'bgGradientStart' }, { input: DOM.inputs.bgGradientEndColor, key: 'bgGradientEnd' },
        { input: DOM.inputs.sidebarBgColor, key: 'sidebarBg' }, { input: DOM.inputs.sidebarTextColor, key: 'sidebarText' },
        { input: DOM.inputs.sidebarHighlightBgColor, key: 'sidebarHighlightBg' }
    ];
    colorInputs.forEach(({ input, key }) => {
        if (input) {
            input.value = draft.theme[key as keyof ChatTheme];
            input.addEventListener('input', (e) => {
                (draft.theme[key as keyof ChatTheme] as any) = (e.target as HTMLInputElement).value;
...
export const renderBrandingSettings = (branding: Branding) => {
    ...
    const addListener = (input: HTMLElement | null, key: keyof Branding | `integrations.${keyof Branding['integrations']}`) => {
        ...
        input.removeEventListener(event, (input as any)._handler);
